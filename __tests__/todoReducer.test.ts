...
describe('todoReducer', () => {
  it('adds a trimmed todo at the top', () => {
    const next = todoReducer(base, {type: 'ADD', id: '3', title: '  Read  ', now: 3});
    expect(next).toHaveLength(3);
    expect(next[0]).toEqual({id: '3', title: 'Read', completed: false, createdAt: 3});
  });

  it('ignores empty titles on add', () => {
    expect(todoReducer(base, {type: 'ADD', id: '3', title: '   ', now: 3})).toBe(base);
  });

  it('edits a todo title', () => {
    const next = todoReducer(base, {type: 'EDIT', id: '1', title: 'Buy bread'});
